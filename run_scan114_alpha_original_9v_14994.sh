...
WORKSPACE="$REPO_DIR/test_DTU/test_scan114_alpha_original/few_shot9/test_M1_repro_14994_seed0"

CUDA_VISIBLE_DEVICES="$GPU_ID" NO_GUI=1 "$PYTHON_BIN" main_nerf.py "$DATASET" \
  --workspace "$WORKSPACE" \
  --split_file "$REPO_DIR/splits/dtu_9v/scan114.json" \
  --few_shot 9 \
  --seed 0 \
  --ckpt scratch \
  --fp16 \
  ...
  --scale 0.33 \
  --bound 2.0 \
  --dataset_name "scan114 9-views Alpha reproduction" \
