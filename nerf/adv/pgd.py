...
def attack_pgd(opt, model, xs, y, perturb_sizes, epsilons, alphas, loss_fn, attack_iters=1, unadv=False, norm="l_inf", restarts=1):
    ...
    for _ in range(restarts):
        ...
        rays_o, rays_d, _, cam_id = xs

        for i_attack in range(max_attack_iters):
            #output = model(*xs, adv_perturb=delta) # add the normalize operation inside model
            output = model.render(rays_o, rays_d, cam_id, adv_perturb=delta, **vars(opt))
            img = output['image'][:, :opt.num_rays]
            loss = loss_fn(img, y)
            if unadv:
                loss = -loss
            loss.mean().backward()
        ...
        output = model.render(rays_o, rays_d, cam_id, adv_perturb=delta, **vars(opt))
        all_loss = loss_fn(output['image'][:, :opt.num_rays], y)
