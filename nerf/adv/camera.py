...
def transform_rays(rays_o, rays_d, cam_id, rotation, offset, num_rays):
    euler = rotation[cam_id]
    tvec = offset[cam_id]
    #rays_o= rays_o+tvec
    rays_o[:num_rays] = rays_o[:num_rays] + tvec
    ...
    rays_d_tmp = rays_d[num_rays:]
    rays_d_rotate = rays_d[:num_rays]

    for R in [Rz, Ry, Rx]:
        rays_d_rotate = torch.matmul(R, rays_d_rotate)
        #rays_d = torch.matmul(R, rays_d)
    rays_d = torch.cat([rays_d_rotate, rays_d_tmp], 0)
